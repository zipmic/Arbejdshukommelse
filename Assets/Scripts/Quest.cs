...
public class Quest : MonoBehaviour {
	...
	private int _currentRoundFound = 0;
	...
	public Text TidBrugt;


	public GameObject BigWall;
	public TimeText TimeTextScript;
    ...
    void Update()
    {
        ...
        {
            _gameStarted = true;
			TimeTextScript.isRunning = true;
			BigWall.GetComponent<Animator>().Play("BigWallMove");
    ...
    public bool CheckWord(string word)
    {
        for (int i = 0; i < WordsToFind.Count; i++)
        {
            if (WordsToFind[i] == word)
            {
				...
				AddCorrectWord();
                if(_currentRoundFound >= AmountOfWordsToFind)
                {
                    // Der er fundet dem alle, new quest og sværere!
                    AmountOfWordsToFind++;
                    NewQuestText();
					_currentRoundFound = 0;
